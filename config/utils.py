...
def save_ckpt(logdir, model,kind):
    if kind=='end':
        ...
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'model_need.pth'))

def load_ckpt(logdir, model, kind):
    if kind=='end':
        save_pth = os.path.join(logdir, 'model_end.pth')
        model.load_state_dict(torch.load(save_pth), False)
    elif kind=='best':
        save_pth = os.path.join(logdir, 'model_best.pth')
        # model.load_state_dict(torch.load(save_pth, map_location={"cuda:2":"cuda:1"}))
